...
func (s *Server) handleGetPortfolioFilterCost() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Println(" Handle Get Portfolio Filter cost has Been Called...")
		body, err := ioutil.ReadAll(r.Body)
		//Unmarshal for funcloc
		hierarchy := FlattenedHierarchyFilter{}
		err = json.Unmarshal(body, &hierarchy)
		...
		var cost float32
		for rows.Next() {
			err = rows.Scan(&a1id, &a1, &a2id, &a2, &a3id, &a3, &a4id, &a4, &a5id, &a5, &a6id, &a6, &cost)
			if err != nil {
				w.WriteHeader(500)
				fmt.Fprintf(w, "Unable to read data from assets List...")
				fmt.Println(err.Error())
			...
			portfolioList := PortfolioListCost{}
			portfolioList.Cost = cost
			portfolioList.Portfolio = append(portfolioList.Portfolio, portfolioListDDlvl1)
			portfolioList.Portfolio = append(portfolioList.Portfolio, portfolioListDDlvl2)
			portfolioList.Portfolio = append(portfolioList.Portfolio, portfolioListDDlvl3)
			portfolioList.Portfolio = append(portfolioList.Portfolio, portfolioListDDlvl4)
...
func (s *Server) handleGetYearReplacementDetails() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			w.WriteHeader(500)
			fmt.Fprintf(w, "Bad JSON provided to Get Year Replacement details")
			return
		}
		...
		for rows.Next() {
			...
			assetsList = append(assetsList, YearReplacementDetails{rulyears, crc})
		}
...
// The function handling the request to get replacementbycondition
...
func (s *Server) handleGetAssetFlexValConditionDetails() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			w.WriteHeader(500)
			fmt.Fprintf(w, "Bad JSON provided to Get flexval by condition details")
			return
		}
		...
		var Assetflexvaluesorted string

		var Crc,
			Drc, Recordcount float32
		...
		for rows.Next() {
			...
			assetsList = append(assetsList, AFVConditionDetails{Crc, Drc, Assetflexvaluesorted, Recordcount})
		}
		...
		if jserr != nil {
			w.WriteHeader(500)
			fmt.Fprintf(w, "Unable to create JSON from Get flexval by condition details result...")
			return
		}
